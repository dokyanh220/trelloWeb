...
const createNewBoardInvitation = async (data) => {
  try {
    ...
    // Biển đồi một số dữ liệu liên quan tới ObjectId chuần chỉnh
...
const findOneById = async (invitationId) => {
  try {
    const result = await GET_DB().collection(INVITATION_COLLECTION_NAME).findOne({ id: new ObjectId
    (invitationId) })
    return result
  } catch (error) { throw new Error(error) }
}
...
const update = async (invitationId, updateData) => {
  try {
    ...
    // Đối với những dữ liệu liên quan ObjectId, biển đồi ở đây
